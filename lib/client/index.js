...
function usePanelOpen() {
    ...
    return [open, function (v) { setOpen(v); setOpenState(v); }];
}
// ---- 工具函数 ----
...
function shortModel(k) { return String(k).split('/').pop(); }
...
const RANGE_DAYS = { week: 7, recent: 30 };
...
function SegControl(props) {
    ...
    return React.createElement('div', { className: 'tds-seg' }, segBtn('week', '近 7 天'), segBtn('recent', '近 30 天'), segBtn('all', '全部'));
}
function KpiCard(props) {
    return React.createElement('div', { className: 'tds-kpi' }, React.createElement('div', { className: 'tds-kpi-top' }, React.createElement('span', { className: 'tds-kpi-icon' }, props.icon), React.createElement('span', { className: 'tds-kpi-label' }, props.label)), React.createElement('div', {
        className: 'tds-kpi-value' + (props.text ? ' tds-kpi-value-text' : ''),
        title: props.title || '',
    }, props.value), props.sub ? React.createElement('div', { className: 'tds-kpi-sub' }, props.sub) : null);
}
function KpiGrid(props) {
    const v = props.v;
    return React.createElement('div', { className: 'tds-kpi-grid' }, KpiCard({ icon: '🪙', label: 'Token 用量', value: formatNumber(v.totalTokens) }), KpiCard({ icon: '💬', label: '消息数', value: formatNumber(v.messages) }), KpiCard({ icon: '🗂', label: '会话数', value: formatNumber(v.sessions) }), KpiCard({ icon: '🧩', label: '模型数', value: formatNumber(v.models) }), KpiCard({ icon: '⭐', label: '最常用模型', value: v.topModelName, text: true, title: v.topModelKey, sub: '占比 ' + v.topModelPct + '%' }), KpiCard({ icon: '📈', label: '日均用量', value: formatNumber(v.avgDaily) }));
}
function HeatmapCard(props) {
    ...
    return React.createElement('div', { className: 'tds-card' }, React.createElement('div', { className: 'tds-card-head' }, React.createElement('span', { className: 'tds-card-title' }, '活跃热力图'), React.createElement('div', { className: 'tds-heat-legend' }, React.createElement('span', null, '较少'), React.createElement('div', { className: 'tds-heat-gradient' }), React.createElement('span', null, '较多'))), content);
}
function StackedBarsCard(props) {
    ...
    if (rows.length === 0 || models.length === 0) {
        ...
        const sums = rows.map(function (r) { let s = 0; for (const k in r.models)
            s += r.models[k]; return s; });
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if (segs.length === 0)
                continue;
            bars.push(React.createElement('g', { key: rows[i].date }, React.createElement('title', null, titleLines.join('\n')), segs));
        }
        const xLabels = [];
        ...
        const legend = React.createElement('div', { className: 'tds-chart-legend' }, models.slice(0, 12).map(function (m, i) {
            return React.createElement('div', { key: m.key, className: 'tds-legend-item' }, React.createElement('span', { className: 'tds-dot', style: { background: colorOf(i) } }), React.createElement('span', { className: 'tds-legend-name' }, shortModel(m.key)));
        }));
        content = React.createElement(React.Fragment, null, React.createElement('svg', { width: '100%', height: H, viewBox: '0 0 ' + W + ' ' + H, style: { display: 'block' } }, gridLines, bars, xLabels), legend);
    }
    return React.createElement('div', { className: 'tds-card' }, React.createElement('div', { className: 'tds-card-head' }, React.createElement('span', { className: 'tds-card-title' }, '按天 Token 趋势')), content);
}
function DonutCard(props) {
    ...
    if (items.length === 0 || total <= 0) {
        ...
        const svg = React.createElement('svg', { width: size, height: size, viewBox: '0 0 ' + size + ' ' + size, style: { flex: 'none' } }, arcs, React.createElement('text', { x: cx, y: cy - 2, textAnchor: 'middle', fontSize: 17, fontWeight: 700, fill: '#1d1d1f' }, formatNumber(total)), React.createElement('text', { x: cx, y: cy + 14, textAnchor: 'middle', fontSize: 9, fill: '#86868b' }, 'tokens'));
        const legend = React.createElement('div', { className: 'tds-donut-legend' }, items.slice(0, 10).map(function (m, i) {
            const pct = Math.round((m.total / total) * 100);
            return React.createElement('div', { key: m.key, className: 'tds-donut-item', title: m.key }, React.createElement('span', { className: 'tds-dot', style: { background: colorOf(i) } }), React.createElement('span', { className: 'tds-donut-name' }, shortModel(m.key)), React.createElement('span', { className: 'tds-donut-val' }, formatNumber(m.total)), React.createElement('span', { className: 'tds-donut-pct' }, pct + '%'));
        }));
        content = React.createElement('div', { className: 'tds-donut-row' }, svg, legend);
    }
    return React.createElement('div', { className: 'tds-card' }, React.createElement('div', { className: 'tds-card-head' }, React.createElement('span', { className: 'tds-card-title' }, '模型用量')), content);
}
function StatsPanel() {
    ...
    if (loading && !data) {
        ...
        body = React.createElement('div', { className: 'tds-error' }, '加载失败：' + error, React.createElement('button', { type: 'button', className: 'tds-btn', onClick: load, style: { marginLeft: 8 } }, '重试'));
    }
    else if (!data) {
        ...
        const diag = showDiag && data._diagnostics
            ...
            : null;
        body = React.createElement(React.Fragment, null, React.createElement('div', { className: 'tds-filter-row' }, React.createElement('span', { className: 'tds-filter-label' }, '时间范围'), React.createElement(SegControl, { value: range, onChange: setRange })), React.createElement(KpiGrid, { v: view.kpi }), React.createElement(HeatmapCard, { data: view.dailyR }), React.createElement(StackedBarsCard, { rows: view.stackedR, models: view.models }), React.createElement(DonutCard, { items: view.models, total: view.totalTokens }), diag);
    }
    return React.createElement('div', { className: 'tds-panel' }, React.createElement('div', { className: 'tds-head' }, React.createElement('span', { className: 'tds-title' }, '📊 Token 统计'), React.createElement('div', { className: 'tds-actions' }, React.createElement('button', { type: 'button', className: 'tds-btn', title: '刷新', onClick: load }, '↻'), React.createElement('button', { type: 'button', className: 'tds-btn', title: '诊断信息', onClick: function () { setShowDiag(!showDiag); } }, '🔍'), React.createElement('button', { type: 'button', className: 'tds-btn', title: '关闭', onClick: function () { setOpen(false); } }, '×'))), React.createElement('div', { className: 'tds-content' }, body));
}
function FabButton() {
...
function DashboardRoot() {
    return React.createElement(React.Fragment, null, React.createElement(FabButton, null), React.createElement(StatsPanel, null));
}
export function apply(ctx) {
    ...
    ctx.effect(() => ctx.slots.inject('shell.overlay', () => ctx.slots.register({ name: 'shell.overlay', id: 'token-dashboard', order: 90 }, () => React.createElement(DashboardRoot))), 'token-dashboard overlay');
}
